...
export function MemorialBookingForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await submitForm({ ...formData, formType: 'Memorial Service' })
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-2xl mx-auto">
      ...
      <Input label="Full Name" name="name" value={formData.name} onChange={handleChange} required placeholder="John Smith" variant="glass" disabled={status === 'submitting'} />

      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Input label="Email Address" name="email" type="email" value={formData.email} onChange={handleChange} required placeholder="john@example.com" variant="glass" disabled={status === 'submitting'} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Select
          label="Service Type" name="serviceType" value={formData.serviceType} onChange={handleChange} required variant="glass" disabled={status === 'submitting'}
          options={[
        ...
        />
        <Input label="Preferred Date" name="date" type="date" value={formData.date} onChange={handleChange} variant="glass" disabled={status === 'submitting'} />
      </div>
