      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6">
        ...
        {!profile ? (
          <div className="rounded-xl border border-gold-100 bg-gold-50/60 p-4">
            <p className="text-sm font-semibold text-slate-800">Complete your professional profile</p>
            <p className="mt-1 text-xs text-slate-600">
              Your account is set up. Complete your professional profile below to get verified access.
            </p>
          </div>
        ) : null}
        ...
        <div className="mt-6 space-y-4">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <input
                type="text"
                defaultValue={profile?.displayName ?? `${user?.firstName ?? ''} ${user?.lastName ?? ''}`.trim()}
