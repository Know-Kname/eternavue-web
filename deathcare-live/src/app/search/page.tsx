...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  const totalResults = listingTotal + billResults.length + postResults.length + profileResults.length
              ...
              {(showAll ? billResults.slice(0, 3) : billResults).map((bill) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="font-semibold text-slate-800 transition-colors group-hover:text-teal-700 line-clamp-1">
                      {bill.title}
                    </h3>
                    <p className="mt-1 line-clamp-2 text-sm text-slate-500">{bill.plainSummary ?? bill.description}</p>
                  </Link>
                )
              })}
            </div>
              ...
              {(showAll ? postResults.slice(0, 3) : postResults).map((post) => (
                ...
                >
                  <div className="mb-2 flex flex-wrap items-center gap-2">
                    ...
                    {post.state && (
                      <span className="text-xs text-slate-400">{post.state}</span>
                    )}
                    <Link
                  ...
                  <p className="text-sm leading-relaxed text-slate-700 line-clamp-3">{post.body}</p>
                </Link>
              ))}
            </div>
          </section>
